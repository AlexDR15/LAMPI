...
SO_VALIDOS="Ubuntu 14.04 y Debian 7"
...
echo ""
echo ""
echo "Para comprobar tu versión del sistema operativo se instalará una dependencia (lsb-release)."
read -e -p "Estás de acurdo? (S):Si, (N):No : " sosino
case $sosino in
	[Ss] ) sudo apt-get install -yqq lsb-release && echo "¡Hecho!";;
	[Nn] ) echo "" && echo "" && echo "Ok, Saliendo..." && echo "" && echo "" && exit;;
	* ) preguntaso
esac
}
preguntaso
...
NOMBRE_OS=$(lsb_release -i | cut -f2)
WEB_OBTENER_IP="icanhazip.com"

# DEPENDE DEL SO, METODO DE OBTENER VERSION_OS
if [ "$NOMBRE_OS" = "Ubuntu" ]; then
	VERSION_OS=$(lsb_release -r | cut -f2)
else
	VERSION_OS=$(lsb_release -r | cut -f2 | cut -c1)
fi
...
echo ""
if [[ "$NOMBRE_OS" = "Ubuntu" && "$VERSION_OS" = "14.04" || "$NOMBRE_OS" = "Debian" && "$VERSION_OS" = "7" ]]; then 
